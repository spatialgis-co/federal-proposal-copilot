...
> **Infrastructure note (persistent):** Outbound network access to `feedback.gsa.gov` is blocked by the cloud execution environment's network policy. `mras_discover.py` and `mras_submitter.py` cannot reach Qualtrics survey URLs from this environment. All survey discovery and form submission must be performed from a local machine or an environment with unrestricted outbound access. This constraint is unresolved since 2026-06-06.

---

## TODAY'S NEW OPPORTUNITIES

**None.** Gmail search `from:rfi@research.gsa.gov newer_than:1d` returned zero threads on 2026-06-07.

---

## CARRIED FORWARD — STILL OPEN

### DOJ FBI — Data Sharing Services Unit Program Support ⚠️ URGENT
| Field | Value |
|---|---|
...
| Due Date | **2026-06-15** — **8 days remaining** |
...
3. **Triage false positive.** "PASS" was triggered by `"NGA"` keyword matching as a substring of "e**nga**gement" — root cause: `mras_triage_classify.py` uses plain substring search (`kw.lower() in text_lower`) without word-boundary enforcement. The actual requirement (data analytics, business consulting, content management, systems integration) has no geospatial/NGA content.
...
**Required action before 2026-06-15:**
...
## TRIAGE CLASSIFIER BUG — ACTION ITEM

`mras_triage_classify.py` uses `kw.lower() in text_lower` (plain substring) for keyword matching. Short acronyms like `NGA`, `GPS`, `GIS` will spuriously match within longer words (e.g., `"NGA"` in `"engagement"`, `"GIS"` in `"logistics"`). This produced the DOJ FBI false positive.

**Recommended fix:** replace `kw.lower() in text_lower` with a word-boundary regex:
```python
import re
...
```
This change will eliminate substring false positives for all acronym keywords.

---

## ACTION ITEMS FOR KENDRICK

1. **URGENT (deadline 2026-06-15 — 8 days):** Review the DOJ FBI DSSU survey manually (link above). See block reasons above and full detail in `daily-queue-2026-06-06.md`.
...
3. **Triage bug fix:** Apply the word-boundary regex fix to `scripts/mras_triage_classify.py` to prevent future NGA/GIS/GPS false positives.
